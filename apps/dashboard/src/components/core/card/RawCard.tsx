import { ActionIcon, Group, Paper, Skeleton, Text, Tooltip } from '@mantine/core';
...
export function RawCard({ title, withEdit = true, withCopy = true, ...props }: RawCardProps) {
					...
					<Group gap="6">
						{props.actions?.map((action, i) => (
							<Tooltip label={action.label} key={'action_' + i}>
								...
								>
									<action.icon style={{ width: '70%', height: '70%' }} stroke={1.5} />
								</ActionIcon>
							</Tooltip>
						))}
						{withCopy && (
						...
						{props.icon && <props.icon className={classes.icon} size="1.4rem" stroke={1.5} />}
					</Group>
				</Group>
				{props.children}
			</Paper>
