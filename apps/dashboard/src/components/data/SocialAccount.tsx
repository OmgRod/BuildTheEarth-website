...
export default function SocialAccount({
  ...
  const generateLinkUrl = async () => {
    ...
    const hashRaw =
      nonce +
      session.data.user.session_state +
      session.data.user.azp +
      ...
      <Flex align={"center"} gap={"md"}>
        ...
        <Flex gap={5} direction={"column"} style={{ flex: 1 }}>
          <Flex align={"center"} gap={"xs"}>
            <Text fw={"bold"}>
              {capitalize(identity?.identityProvider || "")}
            </Text>
          </Flex>
          {!isLinked ? (
            <Badge
...
export function SocialAccountStack({
  ...
  const linkedProviders = identities.map(
    (identity) => identity.identityProvider
  );
  const allProviders = withUnlinked
    ? availableProviders.map((provider) => {
        const identity = identities.find(
          (id) => id.identityProvider === provider
        );
